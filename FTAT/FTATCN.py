...
def is_valid_localization_id(key):
    ...
    if key.lower().replace(":", "") == "l_simp_chinese":
        return False
    if key.endswith("_desc") or key.endswith("_tt") or key.endswith("_TT") or key.endswith("_text"):
        return False
    return True
...
def on_load_localization():
    filepath = filedialog.askopenfilename(filetypes=[("本地化文件", "*.yml")])
    if filepath:
...
def on_load_focus_file():
    filepath = filedialog.askopenfilename(filetypes=[("国策树文件", "*.txt")])
    if filepath:
...
tk.Label(root, text="国策ID列表:").grid(row=4, column=0, sticky='ne')
...
tk.Label(root, text="本地化.yml 预览:").grid(row=10, column=0, sticky='ne')
result_loc = tk.Text(root, height=14, width=60)
result_loc.grid(row=10, column=1, columnspan=4, sticky='w')
tk.Button(root, text="保存本地化.yml", command=on_save_loc).grid(row=11, column=1, sticky='w')
